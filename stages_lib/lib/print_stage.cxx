...
BOOST_DLL_ALIAS_SECTIONED(
  com::stages_lib::PrintStage::make_stage, print_stage, Stages
)
...
namespace com {
  namespace stages_lib {
    ...
    PrintStage::PrintStage(
      const std::string &label, const dag_scheduler::UUID &uuid
    ...
    PrintStage::PrintStage(PrintStage &&other) {
      assert(
        not other.is_running() && "You cannot move a running PrintStage"
      );
    ...
    std::unique_ptr<com::dag_scheduler::TaskStage>
    PrintStage::make_stage(const std::string &name) {
