...
namespace com {
  namespace dag_scheduler {
    ...
    protected:
      ...
      virtual void TearDown() { v_ptr_.reset(); }
    ...
    TEST_F(TestDagVertex, move_ctor_no_edges) {
      ...
      EXPECT_EQ(
        std::string("initialized"), v_moved.current_status_as_string()
      );
    ...
    TEST_F(TestDagVertex, clone_no_edges) {
      ...
      EXPECT_EQ(
        std::string("initialized"), v_cloned.current_status_as_string()
      );
    ...
    TEST_F(TestDagVertex, copy_ctor_no_edges) {
      ...
      EXPECT_EQ(
        std::string("initialized"), v_copied.current_status_as_string()
      );
    ...
    TEST_F(TestDagVertex, visit_all_edges_points_to_actual_verticies) {
      std::vector<std::shared_ptr<DAGVertex>> connections = {
        std::make_shared<DAGVertex>("1"), std::make_shared<DAGVertex>("2"),
        std::make_shared<DAGVertex>("3"), std::make_shared<DAGVertex>("4"),
        std::make_shared<DAGVertex>("5"), std::make_shared<DAGVertex>("6"),
      };
    ...
    TEST_F(TestDagVertex, has_incomming_edge_and_incomming_edge_count) {
      ...
      for (std::shared_ptr<DAGVertex> v : connections) {
        ...
        v->visit_all_edges([&](const DAGEdge &e) {
          ...
          EXPECT_EQ(
            *(e_ref.get_connection().lock()), *(get_dag_vertex_ptr())
          );
        });
      }
