...
class TestTaskCallbackPlugin : public TaskCallbackPlugin,
 ...
 public:
  ...
  virtual void completed(bool completed, Task &task) override {
    Logging::info(LOG_TAG, "completed called with",
                  (completed ? "true" : "false"), "and", task);
  }
...
TEST_F(TestTask, call_back_ctor_sets_appropiate_callback) {
  ...
  std::unique_ptr<Task> test(new TestTaskImpl("test_label", complete_callback));
  EXPECT_NE(nullptr, test);
  EXPECT_NE(test->get_uuid().as_string(), test->label());
  EXPECT_EQ("test_label", test->label());
  test->complete(true);
...
TEST_F(
  ...
  std::unique_ptr<Task> test(new TestTaskImpl("test_label", json_initial_inputs,
                                              json_config, complete_callback));
  EXPECT_NE(nullptr, test);
  EXPECT_NE(test->get_uuid().as_string(), test->label());
  EXPECT_EQ("test_label", test->label());
  test->complete(true);
