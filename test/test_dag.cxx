...
class TestDag : public ::testing::Test, public LoggedClass<TestDag> {
 ...
 protected:
  ...
  std::vector<DAGVertex> fill_dag_default() {
    DAGVertex v0("1"), v1("1"), v2("1a"), v3("1b"), v4("2"), v5("2"), v6("2"),
        v7("3"), v8("3"), v9("4"), v10("5");
    std::vector<DAGVertex> vertices_to_add;
    vertices_to_add.push_back(std::move(v0));
    vertices_to_add.push_back(std::move(v1));
    vertices_to_add.push_back(std::move(v2));
...
TEST_F(TestDag, add_vertices_with_same_and_different_labels) {
  ...
  {
    std::weak_ptr<DAGVertex> v_weak = get_dag().find_vertex(vertices_cloned[0]);
...
TEST_F(TestDag, connect_and_acyclic_check) {
  ...
  EXPECT_EQ(
      0ul,
      get_dag().find_vertex(vertices_cloned[0]).lock()->incomming_edge_count());
  EXPECT_EQ(0ul,
            get_dag().find_vertex(vertices_cloned[1]).lock()->edge_count());
  EXPECT_EQ(
      1ul,
      get_dag().find_vertex(vertices_cloned[1]).lock()->incomming_edge_count());
...
TEST_F(TestDag, add_and_connect_and_acyclic_check) {
...

  {
    ...
    EXPECT_TRUE(test_dag.add_and_connect(std::move(clone0), std::move(clone1)));
  }

  {
    DAGVertex clone0 = vertices_cloned[1].clone();
    DAGVertex clone1 = vertices_cloned[0].clone();
    EXPECT_THROW(test_dag.add_and_connect(std::move(clone0), std::move(clone1)),
                 DAG::DAGException);
  }
...
TEST_F(TestDag, connect_by_uuid) {
...

  {
    ...
    EXPECT_TRUE(test_dag.connect_by_uuid(clone0.get_uuid(), clone1.get_uuid()));
...
TEST_F(TestDag, are_connected_by_uuid) {
...

  {
    ...
    EXPECT_TRUE(get_dag().connect(clone0, clone1));
    EXPECT_TRUE(
        get_dag().are_connected_by_uuid(clone0.get_uuid(), clone1.get_uuid()));
    EXPECT_FALSE(
        get_dag().are_connected_by_uuid(clone1.get_uuid(), clone2.get_uuid()));
  }
