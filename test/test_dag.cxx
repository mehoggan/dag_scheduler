...
namespace com {
  namespace dag_scheduler {
    ...
    TEST_F(
      TestDag, assignment_move_operator_no_edges_no_vertices_title_and_config
    ) {
      rapidjson::Document json_config;
      TestDag::get_generic_config(json_config);
      DAG d("test_dag", json_config);
    ...
    TEST_F(TestDag, connect_and_acyclic_check) {
      ...
      EXPECT_EQ(
        1ul, get_dag().find_vertex(vertices_cloned[0]).lock()->edge_count()
      );
      EXPECT_EQ(
        0ul, get_dag()
      ...
      EXPECT_EQ(
        0ul, get_dag().find_vertex(vertices_cloned[1]).lock()->edge_count()
      );
      EXPECT_EQ(
        1ul, get_dag()
      ...
      EXPECT_THROW(
        get_dag().connect(vertices_cloned[2], vertices_cloned[0]),
        DAG::DAGException
      );
    ...
    TEST_F(TestDag, connect_all_by_label_and_acyclic_check) {
      ...
      EXPECT_THROW(
        get_dag().connect_all_by_label("2", "1"), DAG::DAGException
      );
    ...
    TEST_F(TestDag, connect_by_uuid) {
...

      {
        ...
        std::weak_ptr<DAGVertex> clone0_confirm =
          test_dag.find_vertex(clone0);
        EXPECT_NE(nullptr, clone0_confirm.lock());
        std::weak_ptr<DAGVertex> clone1_confirm =
          test_dag.find_vertex(clone1);
    ...
    TEST_F(TestDag, are_connected_by_uuid) {
...

      {
        ...
        EXPECT_TRUE(get_dag().are_connected_by_uuid(
          clone0.get_uuid(), clone1.get_uuid()
        ));
        EXPECT_FALSE(get_dag().are_connected_by_uuid(
          clone1.get_uuid(), clone2.get_uuid()
        ));
      }
