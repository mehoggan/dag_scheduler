...
namespace com {
  namespace dag_scheduler {
    ...
    TestTaskStageImpl::TestTaskStageImpl(
      const std::string &label, const std::chrono::nanoseconds &run_sleep_time
    ) :
      TaskStage(label), running_(false),
    ...
    TestTaskStageImpl::TestTaskStageImpl(
      ...
      TaskStage(label, uuid), nasty_user_defined_pointer_(new int(10)),
      run_sleep_time_(run_sleep_time) {}

    TestTaskStageImpl::~TestTaskStageImpl() { cleanup(); }

    TestTaskStageImpl::TestTaskStageImpl(TestTaskStageImpl &&other) :
      TaskStage(std::move(other)), running_(false),
