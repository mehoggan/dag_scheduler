...
namespace com {
  namespace dag_scheduler {
    ...
    TEST(TestYAMLDagDeserializer, make_dag_title_dag_config) {
      ...
      EXPECT_EQ(
        expected_config_json_str, get_base_dag_configuration_as_json_str()
      );
    }
    ...
    TEST(TestYAMLDagDeserializer, make_dag_label_config) {
      ...
      yaml_node[YAMLDagDeserializer::DAG_KEY]
               [YAMLDagDeserializer::VERTICES_KEY] =
    ...
    TEST(TestYAMLDagDeserializer, make_dag_vertices_title) {
      ...
      EXPECT_TRUE(
        test_dag->find_vertex_by_uuid(std::move(uuid_1)).lock() != nullptr
      );
      UUID uuid_2(TEST_UUID_1);
      EXPECT_TRUE(
        test_dag->find_vertex_by_uuid(std::move(uuid_2)).lock() != nullptr
      );
    }
    ...
    TEST(TestYAMLDagDeserializer, make_dag_bad_vertices_no_vertex_uuid) {
      ...
      YAML::Node first_vertex;
      first_vertex[YAMLDagDeserializer::NAME_KEY] =
    ...
    TEST(
      TestYAMLDagDeserializer, make_dag_vertices_named_task_initial_inputs
    ) {
      YAML::Node yaml_node;
      yaml_node[YAMLDagDeserializer::DAG_KEY] =
        std::map<std::string, YAML::Node>(
      ...
      first_vertex[YAMLDagDeserializer::TASK_KEY]
                  [YAMLDagDeserializer::INITIAL_INPUTS_KEY] =
                    task_initial_inputs;
      ...
      EXPECT_EQ(
        expected_task_initial_inputs_str, uuid1_task_initial_inputs_str
      );
    }

    TEST(
      TestYAMLDagDeserializer,
    ...
    TEST(
      TestYAMLDagDeserializer, make_dag_label_dag_config_vertices_named_task
    ) {
      YAML::Node dag_configuration_node;
      get_base_dag_configuration(dag_configuration_node);
      YAML::Node yaml_node;
    ...
    TEST(
      TestYAMLDagDeserializer, make_dag_label_vertices_task_initial_inputs
    ) {
      YAML::Node yaml_node;
      yaml_node[YAMLDagDeserializer::DAG_KEY] =
        std::map<std::string, YAML::Node>(
    ...
    TEST(
      ...
      try {
        ...
        auto actual_error_str =
          std::string(yaml_deserialization_error.what());
        EXPECT_TRUE(
          actual_error_str.find(expected_error_substr) != std::string::npos
        );
      }
    }
    ...
    TEST(
      ...
      first_vertex[YAMLDagDeserializer::TASK_KEY]
                  [YAMLDagDeserializer::CALLBACK_KEY]
                  [YAMLDagDeserializer::CALLBACK_TYPE_KEY] =
                    YAML::Node("Function");
      first_vertex[YAMLDagDeserializer::TASK_KEY]
                  [YAMLDagDeserializer::CALLBACK_KEY]
                  [YAMLDagDeserializer::SYMBOL_NAME_KEY] =
    ...
    TEST(
      TestYAMLDagDeserializer, make_dag_label_vertices_task_callback_plugin
    ) {
      YAML::Node yaml_node;
      yaml_node[YAMLDagDeserializer::DAG_KEY] =
        std::map<std::string, YAML::Node>(
    ...
    TEST(TestYAMLDagDeserializer, make_dag_label_vertices_task_stages) {
      ...
      test_vertex->get_task()->iterate_stages([&](TaskStage &stage) -> bool {
        ...
        EXPECT_TRUE(
          actual_type_str.find(expected_type_substr) != std::string::npos
        );
        ++stage_count;
    ...
    TEST(
      TestYAMLDagDeserializer, make_dag_label_vertices_task_stages_task_config
    ) {
      std::string lib_path = testing::TestEnvironment::PATHING.get_lib_path();
      std::string stages_lib_path =
        testing::TestEnvironment::PATHING.get_stages_lib_path();
    ...
    TEST(
      TestYAMLDagDeserializer, make_dag_label_dag_config_vertices_task_stages
    ) {
      std::string lib_path = testing::TestEnvironment::PATHING.get_lib_path();
      std::string stages_lib_path =
        testing::TestEnvironment::PATHING.get_stages_lib_path();
