...
class YAMLDagDeserializerNonSupportedCallbackType : public std::exception {
 public:
  explicit YAMLDagDeserializerNonSupportedCallbackType(const std::string &what);
...
class YAMLDagDeserializer : public LoggedClass<YAMLDagDeserializer> {
 ...
 public:
   ...
   *
   * @param[in] dag_node A \ref YAML::Node to start the deserialization from.
   *
 ...
 private:
  ...
  void make_task_callback(const std::string &task_name,
