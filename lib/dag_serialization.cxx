...
std::unique_ptr<DAG> YAMLDagDeserializer::make_dag(
  ...
  if (dag_node[DAG_KEY]) {
    ...
    if (dag_definition_node[CONFIGURATION_KEY]) {
      ...
      if (json_dag_configuration.Parse<0>(json_str.c_str()).HasParseError()) {
    ...
    if (dag_definition_node[VERTICES_KEY]) {
      ...
      Logging::warn(LOG_TAG, "No\"", VERTICES_KEY, "\" map found in",
                    n2s(dag_node), "Sample input would look like:\n",
                    full_sample_output());
    }
  } else {
    std::string sample = YAMLDagDeserializer::sample_dag_output(UpTo::DAG);
    throw YAMLDagDeserializerError("Root element of \"DAG\" not found in\n" +
                                   n2s(dag_node) + "\nFor example, as in\n" +
                                   sample);
  }
    ...
    if (vertices_node.size() >= 0) {
      for (const YAML::Node &vertex_node : vertices) {
        ...
        try {
          ...
          Logging::error(LOG_TAG, "Error: Missing", UUID_KEY, ". Should",
                         "appear as in", sample_dag_output(UpTo::VERTEX));
        ...
        if (vertex_node[TASK_KEY]) {
          ...
          Logging::warn(LOG_TAG, "A Vertex without a Task was provided", "in",
...
void YAMLDagDeserializer::make_task(const YAML::Node &task_node,
  ...
  if (task_node[CONFIGURATION_KEY]) {
    ...
    Logging::info(LOG_TAG, "Converting", task_node[CONFIGURATION_KEY],
  ...
  if (task_node[INITIAL_INPUTS_KEY]) {
    ...
    Logging::info(LOG_TAG, "Converting", task_node[INITIAL_INPUTS_KEY],
                  "to json string of", buffer.GetString());
  }
...
void YAMLDagDeserializer::make_task_callback(
  ...
  Logging::info(LOG_TAG, "Going to create task callback from", callback_node,
                "...");
    ...
    if (callback_node[CALLBACK_TYPE_KEY]) {
      ...
      if (type == CallbackType::FUNCTION) {
        ...
        if (not task_callback_plugin) {
          ...
          error_stream << "Unable to properly deserialize" << callback_node;
...
std::function<void(bool)> YAMLDagDeserializer::make_task_function_callback(
  ...
  bool symbol_present =
      verify_symbol_present(shared_library, symbol_name, "TaskCb", cb_symbols);
  if (symbol_present && not cb_symbols.empty()) {
    Logging::info(LOG_TAG, "Found", symbol_name, "in",
                  shared_library.get_name());
    ret = shared_library.get_shared_lib().get_alias<void(bool)>(symbol_name);
  } else {
    throw YAMLDagDeserializerError(
        "Failed to load " + symbol_name + " from " + shared_library.get_name() +
        ". It could not be found in available " + " symbols of " + cb_symbols);
  }
  return ret;
}
...
bool YAMLDagDeserializer::verify_symbol_present(
  ...
  Logging::info(
      LOG_TAG, "Going to load", symbol_name, "from", shared_library.get_name(),
  ...
  std::vector<std::string>::iterator find_if_it =
      std::find_if(task_cb_funcs.begin(), task_cb_funcs.end(),
  ...
  Logging::info(
      LOG_TAG, "Going to load", symbol_name, "from", shared_library.get_name(),
      "which contains the following symbols", cb_symbols,
      "and library load status is",
      (shared_library.get_shared_lib().is_loaded() ? "true" : "false"));
...
void YAMLDagDeserializer::load_stages(
  ...
  try {
    ...
    Logging::error(LOG_TAG, "Error: Failed to load stages as a vector.",
      ...
      [this, &out_stages](const YAML::Node &stage_node) {
        ...
        if (stage_node[LIBRARY_NAME_KEY]) {
          ...
          error_stream << std::string("The required key of \"")
                       << LIBRARY_NAME_KEY
                       << std::string("\" was not present in ") << stage_node;
          Logging::error(LOG_TAG, error_stream.str());
          throw YAMLDagDeserializerError(error_stream.str());
        }
...
std::unique_ptr<TaskStage> YAMLDagDeserializer::dynamically_load_stage(
    const DynamicLibraryRegistry::RegistryItem &shared_library,
    const std::string &symbol_name, const std::string &stage_name) const {
  std::string cb_symbols;
  bool symbol_present =
      verify_symbol_present(shared_library, symbol_name, "Stages", cb_symbols);
  if (symbol_present && not cb_symbols.empty()) {
    Logging::info(LOG_TAG, "Found", symbol_name, "in",
                  shared_library.get_name());
