...
namespace com {
  namespace dag_scheduler {
    ...
    const std::string YAMLDagDeserializer::CONFIGURATION_KEY =
      "Configuration";
    ...
    std::unique_ptr<DAG>
    YAMLDagDeserializer::make_dag(const YAML::Node &dag_node) const {
      ...
      if (dag_node[DAG_KEY]) {
        ...
        if (dag_definition_node[CONFIGURATION_KEY]) {
          ...
          Logging::info(
            LOG_TAG, "Converting", dag_definition_node[CONFIGURATION_KEY],
            "to json string of", buffer.GetString()
          );
        }
        ...
        if (dag_definition_node[TITLE_KEY]) {
          ...
          Logging::warn(
            LOG_TAG, "No title specified is this intended?\n",
        ...
        Logging::info(
          LOG_TAG, "Going to build DAG with title --", title,
        ...
        if (dag_definition_node[VERTICES_KEY]) {
          ...
          Logging::warn(
            LOG_TAG, "No\"", VERTICES_KEY, "\" map found in", n2s(dag_node),
            "Sample input would look like:\n", full_sample_output()
          );
        }
      } else {
        std::string sample =
          YAMLDagDeserializer::sample_dag_output(UpTo::DAG);
    ...
    YAMLDagDeserializer::callback_type_from_string(const std::string &enum_str
      ...
      std::transform(
        to_lower.begin(), to_lower.end(), to_lower.begin(),
    ...
    void YAMLDagDeserializer::make_vertices(
      const YAML::Node &vertices_node, std::unique_ptr<DAG> &dag
    ) const {
        ...
        if (vertices_node.size() >= 0) {
          for (const YAML::Node &vertex_node : vertices) {
            ...
            if (vertex_node[TASK_KEY]) {
              ...
              Logging::warn(
                LOG_TAG, "A Vertex without a Task was provided", "in",
                vertex_node
              );
            }
          ...
          Logging::warn(
            LOG_TAG, "Empty verticies node specified to", typeid(*this).name()
          );
        }
      } else {
    ...
    void YAMLDagDeserializer::make_task(
      const YAML::Node &task_node, std::unique_ptr<Task> &task
    ) const {
      ...
      if (task_node[STAGES_KEY]) {
        Logging::info(
          LOG_TAG, "Going to process stages in", task_node[STAGES_KEY], "..."
        );
        load_stages(task_node[STAGES_KEY], stages);
      } else {
        Logging::warn(
          LOG_TAG, "Just created a task from", task_node[TASK_KEY],
          "with no stages"
        );
      }
      ...
      if (task_node[CONFIGURATION_KEY]) {
        ...
        Logging::info(
          LOG_TAG, "Converting", task_node[CONFIGURATION_KEY],
          "to json string of", buffer.GetString()
        );
      }
      ...
      if (task_node[INITIAL_INPUTS_KEY]) {
        ...
        Logging::info(
          LOG_TAG, "Converting", task_node[INITIAL_INPUTS_KEY],
          "to json string of", buffer.GetString()
        );
      }
      ...
      if (task_node[CALLBACK_KEY]) {
        make_task_callback(
          task_name, stages, task_node[CALLBACK_KEY], task, json_config,
          json_initial_inputs
        );
      } else {
    ...
    void YAMLDagDeserializer::throw_wrong_type(
      const UpTo &upto, const std::string &error
    ) const {
      std::string up_to_example =
        YAMLDagDeserializer::sample_dag_output(upto);
      auto error_str = error + std::string(" As in:\n") + up_to_example;
      throw YAMLDagWrongTypeError(error_str);
    }

    std::stringstream &
    operator<<(std::stringstream &ss, std::vector<std::string> sym) {
      std::copy(
        sym.begin(), sym.end(), std::ostream_iterator<std::string>(ss, ",")
      );
      return ss;
    }
    ...
    void YAMLDagDeserializer::make_task_callback(
        ...
        if (callback_node[CALLBACK_TYPE_KEY]) {
          ...
          if (type == CallbackType::FUNCTION) {
            task_callback = make_task_function_callback(
              shared_library, callback_node[SYMBOL_NAME_KEY].as<std::string>()
            );
            task = std::make_unique<Task>(
              stages, task_name, task_callback, json_config,
              json_initial_inputs
            );
            ...
            task = std::make_unique<Task>(
              stages, task_name, std::move(task_callback_plugin), json_config,
              json_initial_inputs
            );
          } else {
      ...
      } catch (std::runtime_error &system_error) {
        std::stringstream error_builder;
    ...
    YAMLDagDeserializer::make_task_function_callback(
      ...
      if (symbol_present && not cb_symbols.empty()) {
        ...
        ret =
          shared_library.get_shared_lib().get_alias<void(bool)>(symbol_name);
      } else {
        throw YAMLDagDeserializerError(
    ...
    void YAMLDagDeserializer::make_task_function_callback_plugin(
      const DynamicLibraryRegistry::RegistryItem &shared_library,
      const std::string &symbol_name, std::unique_ptr<TaskCallbackPlugin> &ret
    ) const {
      std::string cb_symbols;
      bool symbol_present = verify_symbol_present(
        shared_library, symbol_name, "TaskCb", cb_symbols
      ...
      if (symbol_present && not cb_symbols.empty()) {
        ...
        boost::shared_ptr<TaskCallbackPlugin> callback_plugin =
          boost::dll::import_alias<TaskCallbackPlugin>(
            shared_library.get_name(), symbol_name,
    ...
    bool YAMLDagDeserializer::verify_symbol_present(
      const DynamicLibraryRegistry::RegistryItem &shared_library,
      const std::string &symbol_name, const std::string &section_name,
      std::string &cb_symbols
    ) const {
      Logging::info(
        LOG_TAG, "Going to load", symbol_name, "from",
        shared_library.get_name(), "and library load status is",
      ...
      try {
        ...
        throw YAMLDagDeserializerError(
          "Could not load symbol info from " + shared_library.get_name() +
          " with " + rt_error.what()
        );
      ...
      Logging::info(
        LOG_TAG, "Going to load", symbol_name, "from",
        shared_library.get_name(), "which contains the following symbols",
    ...
    void YAMLDagDeserializer::load_stages(
      ...
      std::vector<std::unique_ptr<TaskStage>> &out_stages
    ) const {
      auto stages_vector = stages_node.as<std::vector<YAML::Node>>();
      std::for_each(
        stages_vector.begin(), stages_vector.end(),
        [this, &out_stages](const YAML::Node &stage_node) {
          ...
          try {
            ...
            std::unique_ptr<TaskStage> next_stage =
              dynamically_load_stage(shared_library, symbol_name, stage_name);
            if (next_stage) {
    ...
    std::unique_ptr<TaskStage> YAMLDagDeserializer::dynamically_load_stage(
      const DynamicLibraryRegistry::RegistryItem &shared_library,
      const std::string &symbol_name, const std::string &stage_name
    ) const {
      std::string cb_symbols;
      bool symbol_present = verify_symbol_present(
      ...
      if (symbol_present && not cb_symbols.empty()) {
        ...
        typedef std::unique_ptr<
          TaskStage>(task_stage_creator_t)(const std::string &);
