...
namespace com {
  namespace dag_scheduler {
    ...
    void
    HTTPSSession::SessionResponder::send(Responder::StringMessageType &&msg) {
      auto sp =
      ...
      boost::beast::http::async_write(
        ...
        boost::beast::bind_front_handler(
          &HTTPSSession::on_write, self_.shared_from_this(), sp->need_eof()
        )
      );
    }

    void
    HTTPSSession::SessionResponder::send(Responder::EmptyMessageType &&msg) {
    ...
    HTTPSSession::HTTPSSession(
      ...
      WorkflowService::HTTPSListener &owner, WorkflowService::Router &router
    ) :
      LoggedClass<HTTPSSession>(*this), stream_(std::move(socket), ctx),
      doc_root_(std::move(std::move(doc_root))),
      responder_(new SessionResponder(*this)), owner_(owner),
      router_(router) {}

    void HTTPSSession::run() {
      Logging::info(LOG_TAG, "HTTP session startup...");
      boost::asio::dispatch(
        stream_.get_executor(), boost::beast::bind_front_handler(
    ...
    void HTTPSSession::on_run() {
      ...
      stream_.async_handshake(
        ...
        boost::beast::bind_front_handler(
          &HTTPSSession::on_handshake, shared_from_this()
        )
      );
    }
    ...
    void HTTPSSession::on_read(
      ...
      if (ec == boost::beast::http::error::end_of_stream) {
        ...
        Logging::error(
          LOG_TAG, "Failed to read in session with", ec.message()
        );
      } else {
        ...
        try {
          ...
          if (not handler_ptr) {
            throw std::runtime_error(
              "Failed to handle " + std::string(req_.target())
            );
          } else {
            ...
            if (not status) {
              responder_->send(
                detail::server_error_handler("Internal Server error", req_)
              );
            }
          }
        } catch (std::runtime_error &rte) {
          Logging::error(
            LOG_TAG, "Failed to handle", req_.target(), "with", rte.what()
          );
          responder_->send(detail::not_found_handler(req_.target(), req_));
        }
      }
    }
    ...
    void HTTPSSession::on_write(
      ...
      if (ec) {
        Logging::error(
          LOG_TAG, "Failed to write response with", ec.message()
        );
      } else if (close) {
        do_close();
      } else {
