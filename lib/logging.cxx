...
BOOST_LOG_ATTRIBUTE_KEYWORD(
  severity, "Severity", boost::log::trivial::severity_level
)
...
namespace com {
  namespace dag_scheduler {
    ...
    namespace detail {
      ...
      Logging::Dict::iterator
      find_sink(const LogTag &tag, Logging::Dict &dict) {
        Logging::Dict::iterator it = std::find_if(
          dict.begin(), dict.end(),
          [&](Logging::Dict::value_type &v) {
    ...
    bool Logging::add_std_cout_logger(
      const LogTag &tag, boost::log::trivial::severity_level level
    ) {
      bool ret = false;
    ...
    bool Logging::add_std_cerr_logger(
      const LogTag &tag, boost::log::trivial::severity_level level
    ) {
      bool ret = false;
    ...
    bool Logging::add_std_log_logger(
      const LogTag &tag, boost::log::trivial::severity_level level
    ) {
      bool ret = false;
