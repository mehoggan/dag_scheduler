  ...
  namespace dag_scheduler {
    namespace {
      ...
      make_https_listener(
        ...
        const boost::asio::ip::address &address, const uint16_t port,
    ...
    WorkflowService::WorkflowService(const WorkflowService::ConnectionInfo &ci
      ...
      LoggedClass<WorkflowService>(*this), ioc_(ci.threads_),
      ...
      auto sig_handler =
        [&](const boost::system::error_code &ec, int signum) {
          Logging::info(
            LOG_TAG, "Exiting with", ec.message(), "and signal", signum
          );
          exit(signum);
        };
    ...
    WorkflowService::HTTPSListener::HTTPSListener(
      ...
      LoggedClass<HTTPSListener>(*this), ioc_(ioc), ctx_(ctx), acceptor_(ioc),
      endpoint_(endpoint), doc_root_(doc_root), router_(router) {
      create_acceptor();
    }
